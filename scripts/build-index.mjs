...
function renderReadme(entries, builtAt) {
  const lines = [];
  ...
  lines.push('');
  lines.push(`# ${ORG} — Repository Index`);
  lines.push('');
  lines.push(
...
function renderHtml(entries, builtAt) {
...
<head>
  ...
  <title>${escapeHtml(ORG)} — Repository Index</title>
  <meta name="description" content="An index of repositories in the ${escapeHtml(ORG)} GitHub organization, rebuilt automatically.">
  <script>
...
<body>
  ...
  <header class="border-bottom py-4 mb-4">
    <div class="container">
      <h1 class="h3 mb-1">${escapeHtml(ORG)}</h1>
...
async function main() {
  ...
  await writeFile(path.join(ROOT, 'README.md'), renderReadme(entries, builtAt), 'utf-8');
  await writeFile(path.join(ROOT, 'index.html'), renderHtml(entries, builtAt), 'utf-8');
  await writeFile(
    path.join(ROOT, 'data', 'repos.json'),
    JSON.stringify({ org: ORG, builtAt, repos: entries }, null, 2),
    'utf-8'
  );
